      ...
      <div className="relative h-72 md:h-96 lg:h-[480px]">
        ...
        <div className="absolute bottom-0 left-0 right-0 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
          <nav className="flex items-center gap-2 mb-3 text-sm">
            ...
            <Link href="/event-hall" className="text-sky-300 hover:text-sky-200">Event Hall</Link>
            <span className="text-white/40">/</span>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {hall.suitable.map((s) => (
                  <span key={s} className="px-4 py-2 bg-sky-50 text-sky-700 text-sm rounded-full font-medium border border-sky-100">{s}</span>
                ))}
              </div>
            </div>
            ...
            <div className="sticky top-24 bg-white rounded-2xl shadow-lg border border-slate-100 p-6">
              ...
              <div className="space-y-3 mb-6">
                ...
                <div>
                  ...
                  <input type="number" placeholder="e.g. 200" max={hall.capacity} className="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-sky-300" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide block mb-1">Event Type</label>
                  <select className="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-sky-300 bg-white">
                    {hall.suitable.map((s) => <option key={s}>{s}</option>)}
                  </select>
                </div>
              </div>
              <Link href="/book" className={`flex items-center justify-center w-full py-3 rounded-xl font-semibold text-sm transition-colors ${hall.available ? "bg-[#7B2D3A] text-white hover:bg-[#5C1D28]" : "bg-slate-200 text-slate-500 pointer-events-none"}`}>
                ...
                <div key={r.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-md transition-shadow group">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Link href={`/event-hall/${r.id}`} className="text-sky-600 text-sm font-semibold hover:text-sky-700">View →</Link>
                    </div>
                  </div>
                </div>
              ))}
