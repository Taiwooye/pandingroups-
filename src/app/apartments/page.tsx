...
function categoryLabel(apt: ApiApartment): string {
  return apt.category.label;
}
...
export default function ApartmentsPage() {
  ...
  const filtered =
    active === "All"
      ? apartments
      : apartments.filter((a) => categoryLabel(a).toLowerCase() === active.toLowerCase());
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-20 text-slate-400">
              <p className="font-medium">Failed to load apartments. Please try again.</p>
            </div>
          ) : filtered.length === 0 ? (
            <div className="text-center py-20 text-slate-400">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filtered.map((apt) => {
                const image = apt.media[0]?.url ?? FALLBACK_IMAGE;
                  ...
                  <div key={apt.slug} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group">
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex items-center gap-4 mb-4 text-sm text-slate-600">
                        {apt.bedroom_count != null && (
                          <span className="flex items-center gap-1.5">
                            ...
                            {apt.bedroom_count} {apt.bedroom_count === 1 ? "Bedroom" : "Bedrooms"}
                          </span>
                        )}
                        {apt.size_sqm != null && (
                          <span className="flex items-center gap-1.5">
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                        ...
                        <Link href={`/apartments/${apt.slug}`} className="px-4 py-2 bg-[#5A0E24] text-white text-sm font-semibold rounded-lg hover:bg-[#921224] transition-colors">
                          View Details
                        </Link>
                      </div>
                    </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[
              ...
              { icon: "🛎️", title: "Hotel Services", desc: "Housekeeping every 2 days" },
