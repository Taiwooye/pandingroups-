...
export async function generateMetadata(props: PageProps<"/hotel/[id]">): Promise<Metadata> {
  ...
  try {
    const result = await roomsApi.getBySlug(id);
    const room: ApiRoom = result.data;
...
export default async function HotelRoomPage(props: PageProps<"/hotel/[id]">) {
  ...
  const heroImage = room.media[0]?.url ?? room.media[1]?.url ?? FALLBACK_IMAGE;
  const gallery = room.media.slice(0, 5).map((m) => m.url);
  ...
  return (
    <div className="mt-16 md:mt-20">
      ...
      <div className="relative h-72 md:h-96 lg:h-[480px]">
        <Image src={heroImage} alt={room.name} fill className="object-cover" priority />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <span className="px-3 py-1 rounded-full text-sm font-semibold bg-slate-100 text-slate-700 capitalize">
                {room.category.label}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {[
                { label: "Capacity", value: `${room.max_guests} guests`, icon: "👥" },
                ...(room.size_sqm ? [{ label: "Room Size", value: `${room.size_sqm} m²`, icon: "📐" }] : []),
                { label: "Category", value: room.category.label, icon: "⭐" },
                { label: "Available", value: `${room.available_count} of ${room.total_count}`, icon: "🛏️" },
              ].map((d) => (
            ...
            {room.facilities && room.facilities.length > 0 && (
              <div>
                <h2 className="text-xl font-bold text-slate-800 mb-4">Facilities</h2>
                <div className="flex flex-wrap gap-2">
                  {room.facilities.map((f) => (
                    <span key={f} className="px-3 py-1.5 bg-amber-50 border border-amber-100 rounded-lg text-sm text-amber-700 shadow-sm">
                      {f}
                    </span>
                  ))}
                </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 bg-white rounded-2xl shadow-lg border border-slate-100 p-6">
              ...
              <Link
                href={`/book?service=hotel&id=${room.slug}`}
              ...
              >
                {isAvailable ? "Reserve This Room" : "Not Available"}
              </Link>

              <p className="text-center text-xs text-slate-400 mt-3">No charge until confirmation</p>

              <div className="mt-6 pt-6 border-t border-slate-100 space-y-2">
                {["25% cancellation fee applies", "Complimentary breakfast", "24/7 room service"].map((b) => (
              ...
              {related.map((r) => (
                <div key={r.slug} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-md transition-shadow group">
                  <div className="relative h-44 overflow-hidden">
                    <Image
                      src={r.first_media_url || FALLBACK_IMAGE}
                      alt={r.name}
                      fill
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-amber-700 font-bold">
                        ₦{parseFloat(r.price_per_night).toLocaleString()}
                        <span className="text-slate-400 text-xs font-normal">/night</span>
                      </span>
                      <Link href={`/hotel/${r.slug}`} className="text-amber-600 text-sm font-semibold hover:text-amber-700">View →</Link>
                    </div>
                  </div>
                </div>
              ))}
